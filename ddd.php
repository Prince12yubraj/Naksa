...
if ($driver['status'] === 'rejected' && $driver['is_available'] == 1) {
    ...
    $driver['is_available'] = 0; // Reflect in local variable
}
...
<div class="dashboard">
    ...
    <div class="profile">
        <img src="uploads/drivers/<?php echo htmlspecialchars($driver['driver_photo']); ?>" alt="Driver Photo" />
        <div>
            ...
            <p><strong>Vehicle:</strong> <?php echo htmlspecialchars($driver['vehicle_name'] . ' ' . $driver['vehicle_model']); ?> (<?php echo htmlspecialchars($driver['vehicle_reg']); ?>)</p>
        </div>
    </div>

    <div class="stats">
        <div class="stat-box">
            <a href="driver_list.php"><?php echo $totalBookings; ?></a>
            <p>Total Bookings</p>
        </div>
        <div class="stat-box">
            <a href="driver_list.php?status=pending"><?php echo $bookingStats['pending']; ?></a>
            <p>Pending</p>
        </div>
        <div class="stat-box">
            <a href="driver_list.php?status=accepted"><?php echo $bookingStats['accepted']; ?></a>
            <p>Accepted</p>
        </div>
        <div class="stat-box">
            <a href="driver_list.php?status=completed"><?php echo $bookingStats['completed']; ?></a>
            <p>Completed</p>
        </div>
    </div>
    ...
    <div class="availability">
        <p><strong>Status:</strong>
            ...
            <?php else: ?>
                <span style="color: red;"><?php echo htmlspecialchars(ucfirst($driver['status'])); ?></span>
            <?php endif; ?>
        </p>
        ...
        <?php if ($driver['status'] === 'approved'): ?>
            <a href="?toggle=1" class="btn btn-toggle">
                <i class="fas fa-toggle-<?php echo $driver['is_available'] ? 'on' : 'off'; ?>"></i>
                Go <?php echo $driver['is_available'] ? 'Offline' : 'Online'; ?>
            </a>
        <?php endif; ?>
    </div>
            ...
            <table>
                ...
                <?php while ($booking = $pendingBookings->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($booking['user_message']); ?></td>
                        <td>
                            <button 
                                class="btn-show-route" 
                                data-pickup="<?php echo htmlspecialchars($booking['pickup_location']); ?>" 
                                data-dropoff="<?php echo htmlspecialchars($booking['dropoff_location']); ?>">
                                Show Route
                            </button>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Driver coordinates from server
    let driverCoords = null;

    // Use PHP driver latitude and longitude from DB (fallback if geolocation not allowed)
    <?php if ($driver['current_latitude'] && $driver['current_longitude']): ?>
        driverCoords = [<?php echo $driver['current_latitude']; ?>, <?php echo $driver['current_longitude']; ?>];
    <?php endif; ?>
    ...
    async function showRoute(pickup, dropoff) {
        ...
        if (!map) {
            map = L.map('route-map').setView([27.7172, 85.3240], 13); // Kathmandu default
        ...
        if (!pickupCoords || !dropoffCoords) {
            alert('Could not find pickup or dropoff location.');
            return;
        }
        ...
        dropoffMarker = L.marker(dropoffCoords).addTo(map).bindPopup('Dropoff: ' + dropoff);
        ...
        if (driverCoords) {
            ...
            if (routeDriverPickup) {
                routeLayer1 = L.geoJSON(routeDriverPickup, {color: 'red', weight: 4, opacity: 0.7}).addTo(map);
            } else {
                routeLayer1 = L.polyline([driverCoords, pickupCoords], {color: 'green', weight: 3, dashArray: '5,10'}).addTo(map);
            }
        }

        const routePickupDrop = await getRoute(pickupCoords, dropoffCoords);
        if (routePickupDrop) {
            routeLayer2 = L.geoJSON(routePickupDrop, {color: 'blue', weight: 5, opacity: 0.7}).addTo(map);
        } else {
            routeLayer2 = L.polyline([pickupCoords, dropoffCoords], {color: 'pink', weight: 3, dashArray: '5,10'}).addTo(map);
        }
    ...
    // Update driver location via geolocation API and AJAX
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(updateDriverLocation, locationError, {
    ...
    // Attach route button listeners
    document.querySelectorAll('.btn-show-route').forEach(button => {
        button.addEventListener('click', () => {
            const pickup = button.getAttribute('data-pickup');
            const dropoff = button.getAttribute('data-dropoff');
