...
$sql = "SELECT b.*, u.first_name AS user_name, u.phone AS user_phone 
        FROM bookings b
...
<body>
    <div class="info">
        ...
        <div id="eta_driver">Estimated Time: Calculating you to pickup...</div>
        <div id="eta_ride">Estimated Time: Calculating pickup to dropoff...</div>
        <form method="POST" action="complete_ride.php" onsubmit="return confirm('Are you sure you want to mark this ride as completed?');">
    <input type="hidden" name="booking_id" value="<?= $booking_id ?>">
    <button type="submit" class="complete-btn">Complete Ride</button>
</form>
        ...
        fetch(`https://router.project-osrm.org/route/v1/driving/${driverLng},${driverLat};${pickupLatLng.lon},${pickupLatLng.lat}?overview=false`)
            ...
            .then(data => {
                if (data.routes && data.routes.length > 0) {
                    ...
                    document.getElementById("eta_driver").textContent = "Estimated Time (You → Pickup): " + formatDuration(seconds);
                } else {
                    document.getElementById("eta_driver").textContent = "Unable to calculate ETA (You → Pickup)";
                }
            })
            .catch(() => {
                document.getElementById("eta_driver").textContent = "Unable to calculate ETA (You → Pickup)";
        ...
        fetch(`https://router.project-osrm.org/route/v1/driving/${pickupLatLng.lon},${pickupLatLng.lat};${dropoffLatLng.lon},${dropoffLatLng.lat}?overview=false`)
            ...
            .then(data => {
                if (data.routes && data.routes.length > 0) {
                    ...
                    document.getElementById("eta_ride").textContent = "Unable to calculate ETA (Pickup → Dropoff)";
                }
            })
            .catch(() => {
                document.getElementById("eta_ride").textContent = "Unable to calculate ETA (Pickup → Dropoff)";
    ...
    async function initMap() {
        ...
        L.marker([pickupLatLng.lat, pickupLatLng.lon]).addTo(map).bindPopup("Pickup").openPopup();
